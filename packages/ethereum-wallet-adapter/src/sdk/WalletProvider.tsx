...
import { Address } from '../walletAdapters/BaseWalletAdapter';
...
export const WalletProvider: FC<WalletProviderProps> = ({
  ...
  const sendTransaction = useCallback(
    async (transaction: UnsignedTransaction & { from: Address }) => {
      if (!adapter) throw handleError(new WalletNotSelectedError());
      if (!connected) throw handleError(new WalletNotConnectedError());
      const timeout = timeoutPromise(TIMEOUT * 1000);
      const response = await Promise.race([
        adapter.sendTransaction(transaction),
        timeout.promise
      ]);
      clearTimeout(timeout.timeoutId);
  ...
  const signMessage = useCallback(
    async (msgPayload: string | Uint8Array) => {
      if (!adapter) throw handleError(new WalletNotSelectedError());
      if (!connected) throw handleError(new WalletNotConnectedError());
      const timeout = timeoutPromise(TIMEOUT * 1000);
  ...
  return (
    <WalletContext.Provider
      value={{
        ...
        sendTransaction,
        signMessage,
        network: network as NetworkInfo
      }}>
      {children}
    </WalletContext.Provider>
  );
